...
const INTRO =
  ...
  `> Siana is an editorial atlas of architecture: a curated guide to significant buildings around the world, mapped city by city, paired with Practice — essays, field notes, and practical guides for architects.\n\n` +
  `Siana documents notable architecture across global cities and writes about it with an editorial eye. The site is organized into three areas: **Cities** (city guides with an interactive map of pinned buildings), **Projects** (individual buildings with architect, year and context), and **Practice** (essays, criticism, profiles, interviews, field notes, and practical guides for architecture, engineering and construction practices).\n`;
...
Deno.serve(async () => {
  try {
    ...
    const [cities, projects, posts] = await Promise.all([
      ...
      fetchRows("posts?status=eq.published&select=slug,title,excerpt,category,published_at"),
    ]);
    ...
    const practiceLine = (p: any) =>
      `- [${p.title}](${SITE_URL}/practice/${p.slug}): ${p.category ? p.category + " — " : ""}${p.excerpt ?? ""}`.trimEnd();
    ...
    const body =
      ...
      `## Practice\n${practice.map(practiceLine).join("\n")}\n\n` +
      `## Projects\n${projects.map(projectLine).join("\n")}\n\n` +
      `## Main sections\n` +
      `- [Cities index](${SITE_URL}/cities): Browse all cities in the atlas.\n` +
      `- [Practice](${SITE_URL}/practice): Essays, field notes, and practical guides for architecture, engineering and construction practices.\n` +
