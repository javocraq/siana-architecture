...
- **Rich text** — authored with TipTap (`src/components/admin/RichTextEditor.tsx`, includes the table extensions) and rendered through `src/components/site/RichHtml.tsx`, which sanitizes with DOMPurify. Sanitize any DB-sourced HTML the same way.
