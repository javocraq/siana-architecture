...
- **Newsletter** — `NewsletterCta` (in `SiteLayout`, so it is the footer of every page) posts to the `newsletter-subscribe` Edge Function; the reader is never sent to substack.com. The function stores the address in `newsletter_subscribers` **first**, then forwards it to `sianaarchitecture.substack.com`. That order matters: Substack has no documented public subscribe API, so the hand-off uses the internal endpoint their embed posts to and may break without notice — `substack_status` records the outcome per row so `failed` ones can be retried or exported. Only the Edge Function writes to the table (service role); RLS grants admins read access and nobody else. Never make the form claim success before the backend confirms it.
